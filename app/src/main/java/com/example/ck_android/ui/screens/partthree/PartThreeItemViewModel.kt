...
@HiltViewModel
class PartThreeItemViewModel @Inject constructor(
    private val apiService: ApiService,
    private val mainLog: MainLog,
    private val dataStoreManager: DataStoreManager,
) : ViewModel() {
    val _uiState = MutableStateFlow(PartThreeQuestionResponse())
    val uiState = _uiState.asStateFlow()

    val _scoreExam = MutableStateFlow(ScoreResponse())
    val scoreExam = _scoreExam.asStateFlow()

    fun getPartThreeQuestion(id: String) {
        viewModelScope.launch {
            _uiState.value = _uiState.value.copy(status = LoadStatus.Loading())
            try {
                val accessToken = dataStoreManager.getAccessToken().first()
                val response =
                    requireNotNull(apiService).getQuestionPartThree("Bearer $accessToken", id)
